...
btnTambah.addEventListener('click', async ()=>{
    ...
    const {value : formValues} = await Swal({
        ...
        '<label class="text-left">Nama Penghasilan</label>'+
        '<input type="text" id="input-penghasilan" class="form-control">'+
        '</div>',
        focusConfirm: false,
        preConfirm: () => {
            return {"nama" : document.getElementById('input-penghasilan').value}
        }
    })
...
$(document).ready(function(){
    ...
    let t = $('#tablekategori').DataTable({
        ...
        columns : [
            {"data": null},
            {"data": "id_penghasilan", "width": "17%"},
            {"data": "nama_penghasilan", "width": "62%"},
            {"data": null,
    ...
    $('#tablekategori tbody').on('click', '#delete', function() {
        ...
        const id = data[0].id_penghasilan;
...
const popupEdit = async (data) => {
    ...
    for (let i = 0; i < data.length; i++) {
        id = data[i].id_penghasilan;
        nama = data[i].nama_penghasilan;
    }
    ...
    const {value: formValues} = await Swal({
        ...
        html : 
        '<div class="form-group text-left" >'+
        '<label class="text-left">ID Penghasilan </label>'+
        '<input type="text" id="input-id" class="form-control" value="'+id+'" readonly>'+
        '</div>'+
        '<div class="form-group text-left">'+
        '<label>Nama Penghasilan</label>'+
        '<input type="text" id="input-penghasilan" class="form-control" value="'+nama+'" >'+
        '</div>',
        focusConfirm: false,
        preConfirm : () => {
            return {
                "id" : document.getElementById('input-id').value,
                "nama" : document.getElementById('input-penghasilan').value
            }
        }
    })
...
const inputEditData = (dataInput) => {
    $.ajax ({
        url: 'http://localhost/Perpustakaan/murid/editPenghasilan',
        type: 'post',
        data : dataInput,
        success : () => {
...
const alertHapus = (id)=>{
    ...
    swalWithBootstrap({
        ...
        if (result.value) {
            $.ajax({
                url: 'http://localhost/Perpustakaan/murid/hapusPenghasilan',
                method : 'post',
                data : {id : id},
                success: () => {
                    $('#tablekategori').DataTable().ajax.reload();
...
const saveData = (dataInput)=>{
    $.ajax({
        url : 'http://localhost/Perpustakaan/murid/insertpenghasilan',
        type: 'POST',
        data : dataInput,
        success : () => {
            $('#tablekategori').DataTable().ajax.reload();
