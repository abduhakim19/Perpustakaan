...
btnTambah.addEventListener('click', async ()=>{
    ...
    const {value : formValues} = await Swal({
        ...
        '<label class="text-left">Nama Jurusan</label>'+
        '<input type="text" id="input-jurusan" class="form-control">'+
        '</div>',
        focusConfirm: false,
        preConfirm: () => {
            return {"nama" : document.getElementById('input-jurusan').value}
        }
    })
...
$(document).ready(function(){
    ...
    let t = $('#tablekategori').DataTable({
        ...
        columns : [
            {"data": null},
            {"data": "id_jurusan", "width": "14%"},
            {"data": "nama_jurusan", "width": "62%"},
            {"data": null,
...
const inputEditData = (dataInput) => {
    $.ajax ({
        url: 'http://localhost/Perpustakaan/murid/editjurusan',
        type: 'post',
        data : dataInput,
        success : () => {
...
const alertHapus = (id)=>{
    ...
    swalWithBootstrap({
        ...
        if (result.value) {
            $.ajax({
                url: 'http://localhost/Perpustakaan/murid/hapusjurusan',
                method : 'post',
                data : {id : id},
                success: () => {
                    $('#tablekategori').DataTable().ajax.reload();
...
const saveData = (dataInput)=>{
    ...
    $.ajax({
        url : 'http://localhost/Perpustakaan/murid/insertjurusan',
        type: 'POST',
        data : dataInput,
        success : () => {
            $('#tablekategori').DataTable().ajax.reload();
